...
# 🛠️ Pegar chave da API Perplexity do Streamlit Secrets
...
# 🔵 2️⃣ DASHBOARD
    ...
    # Exibir Tabela de Monitoramento
    ...
    # 🔵 Identificação de Padrões e Monitoramento Contínuo
...
# 🟠 3️⃣ DATA LAB (Análise Semântica com InfraNodus)
elif menu == "Data Lab":
...
# 🟣 4️⃣ DECISION MAKE (Tomada de Decisão + IA Perplexity)
    ...
    pergunta_zaia = st.text_input("Pergunte algo para Zaia:")

    if st.button("Enviar Pergunta"):
        resposta_zaia = "Zaia ainda está aprendendo! Em breve, terá respostas mais inteligentes."
        st.write(resposta_zaia)

    # 🗣️ Chat com API da Perplexity
    st.subheader("🗣️ Chat com Perplexity API")

    pergunta_perplexity = st.text_input("Faça uma consulta à Perplexity AI:")
    ...
    if st.button("Consultar Perplexity"):
        ...
        data = {
            "model": "mistral",
            "messages": [{"role": "user", "content": pergunta_perplexity}]
        }
